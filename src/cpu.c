...
uint16_t* cpu_r16_ptr(struct gbc* dev, enum cpu_reg_r16 r16)
{
    switch (r16)
    {
        ...
        case R16_SP: return &dev->cpu.SP.val; break;
    }
}
...
uint16_t cpu_r16_val(struct gbc* dev, enum cpu_reg_r16 r16)
{
    switch (r16)
    {
        ...
        case R16_SP: return dev->cpu.SP.val;
    }
}
...
{
    return dev->mem[dev->cpu.PC.val++];
}

uint16_t cpu_fetch_word(struct gbc* dev)
{
    return dev->mem[dev->cpu.PC.val++] | (dev->mem[dev->cpu.PC.val++] << 4);
}